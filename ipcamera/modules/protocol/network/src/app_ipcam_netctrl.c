    ...
    for(int i = 0; i < APP_VENC_CHN_NUM; i++) {
        ...
        if (stResetVencFlag[i].bResolution) {
            ...
            if (i == APP_VENC_STREAM_SUB) {
                ...
                APP_VPSS_GRP_CFG_T *pstMdVpssGrpCfg = &app_ipcam_Vpss_Param_Get()->astVpssGrpCfg[VpssGrp_MD];
...
{
    if(app_ipcam_MD_ProcStatus_Get() != psmdinfo.enabled)
    {
        if (psmdinfo.enabled)
        {
            app_ipcam_MD_Start();
    ...
    if(app_ipcam_MD_Thresold_Get() != psmdinfo.threshold)
    {
        printf("md1:%d\n",app_ipcam_MD_Thresold_Get());
        app_ipcam_MD_Thresold_Set(psmdinfo.threshold);
        printf("md:%d\n",app_ipcam_MD_Thresold_Get());
        return 0;
    }
