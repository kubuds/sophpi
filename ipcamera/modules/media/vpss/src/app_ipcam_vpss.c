...
int app_ipcam_Vpss_Create(VPSS_GRP VpssGrp)
{
    ...
    CVI_BOOL bMipiSwitchEnable = pstSysCfg->astSwitchCfg.bMipiSwitchEnable;
    VPSS_GRP iSwitchPipe1 = pstSysCfg->astSwitchCfg.u32SwitchPipe1;
#endif
    ...
    for (VPSS_CHN VpssChn = 0; VpssChn < VPSS_MAX_PHY_CHN_NUM; VpssChn++) {
        ...
        if (pstVpssGrpCfg->abChnEnable[VpssChn]) {
...
#else
            if (bSBMEnable){
                if(((VpssGrp == 0) && (VpssChn == 0)) || ((bMipiSwitchEnable) && (VpssGrp == iSwitchPipe1) && (VpssChn == 0))){
                    ...
                    stVpssChnBufWrap.u32WrapBufferSize = pstSysCfg->astSBMCfg.slice_buf_num;
            ...
            if (pastDwaCfg->astDwaCfg[VpssGrp].enRotation > 0 && pastDwaCfg->bUserEnable == 0) {
                s32Ret = CVI_VPSS_SetChnRotation(VpssGrp, VpssChn, pastDwaCfg->astDwaCfg[VpssGrp].enRotation);
                if (s32Ret != CVI_SUCCESS) {
            ...
            if (pastDwaCfg->astDwaCfg[VpssGrp].FisheyeAttr.bEnable && pastDwaCfg->bUserEnable == 0) {
                APP_PROF_LOG_PRINT(LEVEL_INFO, "vpss grp[%d] chn:[%d] start fisheye\n", VpssGrp, VpssChn);
                s32Ret = CVI_VPSS_SetChnFisheye(VpssGrp, VpssChn, &pastDwaCfg->astDwaCfg[VpssGrp].FisheyeAttr);
                if (s32Ret != CVI_SUCCESS) {
