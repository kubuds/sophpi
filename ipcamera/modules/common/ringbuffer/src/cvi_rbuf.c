...
void cvi_rbuf_show_meminfo(void *rbuf) {
    ...
    APP_PROF_LOG_PRINT(LEVEL_DEBUG,"%p wcnt %"PRIu64", rcnt %"PRIu64", wpos %#lx, rpos %#lx, bsize %u, rsize %u", rbuf, buf->wcnt, buf->rcnt, buf->wpos, buf->rpos, buf->size, rmsize);
}
...
void cvi_rbuf_show_log(void *rbuf){
    ...
    if(buf){
        APP_PROF_LOG_PRINT(LEVEL_DEBUG,"ringbuf %p %08d %016llu %08llu %016llu %08llu %016llu %08llu\r\n", rbuf, buf->size,
...
int cvi_rbuf_init(void **rbuf, int size, const char *name) {
    ...
    if(buf == NULL) {
        *rbuf = NULL;
        APP_PROF_LOG_PRINT(LEVEL_ERROR,"out of mem size %d name %s\r\n", size, name);
        return -1;
    }
    ...
    if(!buf->data) {
        APP_PROF_LOG_PRINT(LEVEL_ERROR,"rbuf create failed! the args size %d name %s\r\n", size, name);
    ...
    buf->size = s;
    *rbuf = buf;
    APP_PROF_LOG_PRINT(LEVEL_DEBUG,"rbuf %s is created, size %d\r\n", buf->name, buf->size);
    return 0;
}
...
void cvi_rbuf_deinit(void *rbuf) {
    ...
    if(buf) {
        ...
        APP_PROF_LOG_PRINT(LEVEL_DEBUG,"rbuf %s is free, size %d\r\n", buf->name, buf->size);
        free(buf);
    }
}
