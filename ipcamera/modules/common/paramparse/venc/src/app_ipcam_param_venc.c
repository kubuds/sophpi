...
#ifdef __CV186AH__
    [VENC_BIND_DPU] = "VENC_BIND_DPU",
    [VENC_BIND_STITCH] = "VENC_BIND_STITCH",
#endif
};
...
int Load_Param_Venc(const char *file)
{
    ...
    for (i = 0; i < chn_num; i++) {
        ...
        /* VENC not bind other module*/
#ifdef __CV186AH__
        if (Venc->astVencChnCfg[i].enBindMode == VENC_BIND_DISABLE) {
            APP_PROF_LOG_PRINT(LEVEL_INFO, "bind_mode=%d\n", Venc->astVencChnCfg[i].enBindMode);

            ini_gets(tmp_section, "frm_src_mod_id", " ", str_name, PARAM_STRING_NAME_LEN, file);
            ret = app_ipcam_Param_Convert_StrName_to_EnumNum(str_name, mode_id, CVI_ID_BUTT, &enum_num);
            if (ret != CVI_SUCCESS) {
                APP_PROF_LOG_PRINT(LEVEL_INFO, "[%s][frm_src_mod_id] Fail to convert string name [%s] to enum number!\n", tmp_section, str_name);
            } else {
                APP_PROF_LOG_PRINT(LEVEL_INFO, "[%s][frm_src_mod_id] Convert string name [%s] to enum number [%d].\n", tmp_section, str_name, enum_num);
                Venc->astVencChnCfg[i].stVencFrameSourceCfg.enModId = enum_num;
            }

            Venc->astVencChnCfg[i].stVencFrameSourceCfg.s32DevId = ini_getl(tmp_section, "frm_src_dev_id", 0, file);
            Venc->astVencChnCfg[i].stVencFrameSourceCfg.s32ChnId = ini_getl(tmp_section, "frm_src_chn_id", 0, file);
        }
#endif
