...
__attribute__((weak)) int Load_Param_Dwa(const char *file)
{
    APP_PROF_LOG_PRINT(LEVEL_INFO, "%s defalut param \r\n", __func__);
    return 0;
}
...
#ifdef __CV186AH__
    [CVI_ID_DWA] = "CVI_ID_DWA",
#endif
    [CVI_ID_PHOTO] = "CVI_ID_PHOTO",
    [CVI_ID_FB] = "CVI_ID_FB",
#ifdef __CV186AH__
    [CVI_ID_DPU] = "CVI_ID_DPU",
#endif
#ifdef __CV186AH__
    [CVI_ID_STITCH] = "CVI_ID_STITCH",
    [CVI_ID_HDMI] = "CVI_ID_HDMI",
#endif
};
...
int app_ipcam_Param_Load(void)
{
    ...
    APP_CHK_RET(Load_Param_Dwa(ParamCfgFile), "Load DWA Param");
