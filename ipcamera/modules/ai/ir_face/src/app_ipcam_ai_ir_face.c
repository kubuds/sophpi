...
static cvitdl_handle_t g_AI_Handle = NULL;
...
static CVI_S32 _list_push_back(APP_IRFACE_FEATURE_LIST_S * pstHead, cvtdl_feature_t * pstNode, char * name)
{
...
{
    //CVI_S32 s32Ret = CVI_TDL_CreateHandle2(&g_AI_Handle, AI_VPSSGRP, AI_VPSSDEV);
    //初始化句柄
    CVI_S32 s32Ret = CVI_TDL_CreateHandle(&g_AI_Handle);
    if (s32Ret != CVI_SUCCESS) {
        APP_PROF_LOG_PRINT(LEVEL_ERROR, " CVI_TDL_CreateHandle err ret %d \r\n", s32Ret);
        return s32Ret;
    }
    ...
    if (app_ipcam_Ai_IR_FD_Param_Get()->bVpssPreProcSkip) {
        //预处理
        CVI_TDL_SetSkipVpssPreprocess(g_AI_Handle, CVI_TDL_SUPPORTED_MODEL_SCRFDFACE, app_ipcam_Ai_IR_FD_Param_Get()->bVpssPreProcSkip);
        CVI_TDL_SetSkipVpssPreprocess(g_AI_Handle, CVI_TDL_SUPPORTED_MODEL_IRLIVENESS, app_ipcam_Ai_IR_FD_Param_Get()->bVpssPreProcSkip);
        CVI_TDL_SetSkipVpssPreprocess(g_AI_Handle, CVI_TDL_SUPPORTED_MODEL_FACERECOGNITION, app_ipcam_Ai_IR_FD_Param_Get()->bVpssPreProcSkip);
    }
    if (app_ipcam_Ai_IR_FD_Param_Get()->attachPoolId != -1) {
        s32Ret = CVI_TDL_SetVBPool(g_AI_Handle, 0, app_ipcam_Ai_IR_FD_Param_Get()->attachPoolId);
    }
    return CVI_SUCCESS;
}

static CVI_S32 app_ipcam_Ai_IR_FD_Recognition(VIDEO_FRAME_INFO_S *pstFrame, cvtdl_face_t * pstFaceResult)
{
    int ret = 0;
    //IR人脸识别
    ...
    ret = CVI_TDL_ScrFDFace(g_AI_Handle, pstFrame, pstFaceResult);
    if (ret != CVI_SUCCESS) {
        APP_PROF_LOG_PRINT(LEVEL_ERROR, "CVI_TDL_ScrFDFace failed ret %#x!\r\n", ret);
        return CVI_FAILURE;
    }
    if (pstFaceResult->size != 1) {
        //APP_PROF_LOG_PRINT(LEVEL_ERROR, "CVI_TDL_ScrFDFace err face_meta.size %d\r\n", pstFaceResult->size);
        return CVI_FAILURE;
    }
    CVI_TDL_IrLiveness(g_AI_Handle, pstFrame, pstFaceResult);
    CVI_TDL_FaceRecognition(g_AI_Handle, pstFrame, pstFaceResult);
    return CVI_SUCCESS;
}
...
CVI_S32 app_ipcam_Ai_IR_FD_Register(char * gallery_name)
{
    ...
    CVI_TDL_ScrFDFace(g_AI_Handle, &stFrame, &stFaceMeta);
    ...
    //活体检测
    CVI_TDL_IrLiveness(g_AI_Handle, &stFrame, &stFaceMeta);
    ...
    // 人脸识别
    CVI_TDL_FaceRecognition(g_AI_Handle, &stFrame, &stFaceMeta);
    CVI_VPSS_ReleaseChnFrame(VpssGrp, VpssChn, &stFrame);
    pthread_mutex_unlock(&g_IrFaceMutex);
    // 保存人脸底库
    if (stFaceMeta.size == 1) {
...
{
    if (g_AI_Handle != NULL) {
        CVI_TDL_DestroyHandle(g_AI_Handle);
        g_AI_Handle = NULL;
    }
    return CVI_SUCCESS;
}
